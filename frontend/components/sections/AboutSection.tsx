...
export default function AboutSection() {
  ...
  useEffect(() => {
    publicApi.getTeamMembers().then(setTeam).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <section id="about" ref={ref} className="py-12 sm:py-16 md:py-20 bg-white dark:bg-gray-900 transition-colors duration-300">
      <div className="container mx-auto px-4">
        ...
        {/* الرؤية والرسالة (من ملف PDF) */}
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6 mb-10">
          <motion.div ... className="bg-white dark:bg-gray-800 p-4 sm:p-6 md:p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        {/* صور الفريق (ديناميكية من قاعدة البيانات) */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {team.map((member) => (
                <div key={member.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 text-center">
                  {member.image && (
                    <img src={member.image} alt={member.name} className="w-24 h-24 rounded-full mx-auto mb-3 object-cover border-2 border-[#01AEBE] dark:border-[#00c6ff]" />
                  )}
